...
import { KrnBadge, KrnCard } from '../data-display/display';
...
@Component({
  ...
  styles: `
    ...
    .trigger {
      ...
      cursor: pointer;
      transition:
        border-color var(--krn-motion-fast, 120ms) var(--krn-ease-standard, ease),
        background-color var(--krn-motion-fast, 120ms) var(--krn-ease-standard, ease);
    }
    .trigger:hover,
    ...
    .chevron {
      ...
      transition:
        rotate var(--krn-motion-fast, 120ms) var(--krn-ease-standard, ease),
        translate var(--krn-motion-fast, 120ms) var(--krn-ease-standard, ease);
    }
    ...
    .menu {
      ...
      animation: user-menu-in var(--krn-motion-fast, 120ms) var(--krn-ease-enter, ease-out);
  ...
  template: `
    <section [attr.aria-label]="ariaLabel()">
      <header>
        <span>
          ...
          <krn-badge [tone]="unreadCount() ? 'brand' : 'neutral'"
            >{{ unreadCount() }} unread</krn-badge
          >
        </span>
        @if (unreadCount()) {
          <button type="button" (click)="markAllRead.emit()">Mark all read</button>
        }
      </header>
      @if (notifications().length) {
        ...
        <div class="empty" role="status">You are all caught up.</div>
      }
    </section>
  `,
  styles: `
...
@Component({
  ...
  template: `
    <div class="searchbox">
      ...
      />
      @if (query()) {
        <button type="button" aria-label="Clear search" (click)="clear()">×</button>
      }
    </div>
    @if (open() && query()) {
      <div
      ...
      >
        @if (filteredResults().length) {
          ...
          <p role="status">No results for “{{ query() }}”</p>
        }
      </div>
    }
  `,
...
export class KrnGlobalSearch {
  readonly ariaLabel = input('Global search');
  readonly placeholder = input('Search…');
  readonly results = input<readonly KrnSearchResult[]>([]);
  readonly maxResults = input(8);
  readonly resultsId = input('krn-global-search-results');
  ...
  readonly activeResultId = computed(() => {
    ...
    return result ? `${this.resultsId()}-${result.id}` : null;
  });
  ...
  choose(result: KrnSearchResult): void {
...
@Component({
  ...
  template: `
    <div class="filters" [attr.aria-label]="ariaLabel()" role="group">
      ...
      @if (activeCount()) {
        <krn-badge tone="brand">{{ activeCount() }} active</krn-badge>
...
export class KrnFilterBar {
  ...
  readonly activeCount = computed(() => Object.values(this.values()).filter(Boolean).length);

  setFilter(id: string, event: Event): void {
...
@Component({
  ...
  template: `
    @if (open()) {
      <div class="backdrop" aria-hidden="true" (click)="open.set(false)"></div>
      <aside
        role="dialog"
        aria-modal="true"
      ...
      >
        <header>
          <strong>{{ heading() }}</strong>
          <button type="button" aria-label="Close settings" (click)="open.set(false)">×</button>
        </header>
        <div class="body"><ng-content /></div>
        <footer><ng-content select="[krnSettingsActions]" /></footer>
      </aside>
    }
  `,
  styles: `
    ...
    aside {
      ...
      color: var(--krn-color-text, #252932);
      background: var(--krn-color-surface, #fff);
      box-shadow: var(--krn-shadow-drawer, -16px 0 40px rgb(20 24 32 / 0.18));
      animation: enter var(--krn-motion-slow, 240ms)
...
export class KrnSettingsPanel {
  readonly heading = input('Settings');
  readonly open = model(false);
}
...
export class KrnMasterDetailLayout {
  readonly masterLabel = input('Master list');
  readonly detailLabel = input('Detail');
...
export class KrnMobileNavigation {
  readonly ariaLabel = input('Mobile navigation');
}
  ...
  styles: `
    ...
    @media (max-width: 48rem) {
      ...
      aside {
        ...
        translate: -102% 0;
        transition: translate var(--krn-motion-slow, 240ms)
          var(--krn-ease-enter, cubic-bezier(0.16, 1, 0.3, 1));
      }
