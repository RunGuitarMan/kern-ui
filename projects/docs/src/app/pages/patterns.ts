  ...
  template: `
    <article class="page">
      ...
      <section class="pattern">
        <header>
          ...
          <p>Selection state changes the toolbar context. Narrow containers replace the two-column split with a list/detail transition.</p>
        </header>
        <div class="demo-frame">
          <krn-crud-toolbar ariaLabel="Workspace actions" [selectedCount]="selectedCount()">
            <strong krnToolbarTitle>Workspaces</strong>
            @if (selectedCount()) {
              <krn-button variant="ghost" tone="danger" (activated)="selectedCount.set(0)">Archive</krn-button>
            } @else {
          ...
          <krn-master-detail-layout [(detailOpen)]="detailOpen">
            ...
            <div krnDetail class="detail-panel">
              <button class="back" type="button" (click)="detailOpen.set(false)">← Back to workspaces</button>
              <span class="eyebrow">WORKSPACE / {{ selectedWorkspace() }}</span>
              <h3>{{ selectedWorkspaceName() }}</h3>
              <p>Permissions, automation health, and usage context remain available without leaving the list.</p>
              <krn-alert tone="success" title="Healthy">All scheduled runs completed inside the service target.</krn-alert>
            </div>
      ...
      <section class="pattern">
        <header>
          ...
          <p>Widgets are not decorative cards: heading, data, status, and actions remain structurally consistent.</p>
        </header>
      ...
      <section class="pattern">
        <header>
          ...
          <p>Navigation exposes the current step, completed reach, optional labels, and invalid-step gating.</p>
        </header>
        <div class="form-surface">
          ...
          >
            @switch (currentStep()) {
              @case (0) {
                <div class="step-copy"><span>STEP 01</span><h3>Name the workspace</h3><p>Choose a name your team can recognize in audit logs.</p></div>
              }
              @case (1) {
                <div class="step-copy"><span>STEP 02</span><h3>Invite collaborators</h3><p>Add members now, or continue and invite them later.</p></div>
              }
              @case (2) {
                <div class="step-copy"><span>STEP 03</span><h3>Review policy</h3><p>Confirm access and retention settings before creation.</p></div>
              }
            }
          </krn-multi-step-form>
          @if (formCompleted()) {
            <krn-alert tone="success" title="Workspace created">The pattern emitted a typed completion event.</krn-alert>
          }
        </div>
      </section>
      ...
      <section class="pattern auth-pattern">
        <header>
          ...
          <p>Touched, invalid, dirty, loading, error, and saved states are visible to both users and assistive technology.</p>
        </header>
        <div class="auth-grid">
          <div class="form-card">
            ...
            @if (loginMessage()) {
              <krn-alert tone="success" title="Credentials accepted">{{ loginMessage() }}</krn-alert>
            }
          </div>
          <div class="form-card">
      ...
      <section class="pattern compact-pattern">
        ...
        <div class="context-demo">
          <krn-user-menu name="Avery Cole" detail="avery@north.ops">
            ...
            <button role="menuitem" type="button" (click)="settingsOpen.set(true)">Workspace settings</button>
            <button role="menuitem" type="button">Sign out</button>
          </krn-user-menu>
          <krn-button variant="outline" tone="neutral" (activated)="settingsOpen.set(true)">Open settings</krn-button>
        </div>
      </section>
    </article>
    ...
    <krn-settings-panel heading="Workspace settings" [(open)]="settingsOpen">
      <div class="settings-content">
        ...
        <label>Data region <select><option>European Union</option><option>United States</option></select></label>
        <label class="check"><input type="checkbox" checked /> Require SSO for administrators</label>
      </div>
      <div krnSettingsActions>
        <krn-button variant="outline" tone="neutral" (activated)="settingsOpen.set(false)">Cancel</krn-button>
...
export class PatternsPage {
  ...
  protected readonly selectedWorkspaceName = () =>
    this.workspaces.find((workspace) => workspace.id === this.selectedWorkspace())?.name ?? 'Workspace';
